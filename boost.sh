...
optimize_network() {
    ...
    cat > /etc/sysctl.d/99-luk1s-net.conf << 'EOF'
# BBR
...
EOF
    ...
    sysctl -p /etc/sysctl.d/99-luk1s-net.conf >/dev/null 2>&1
    
    # File limits
    cat > /etc/security/limits.d/99-luk1s.conf << 'EOF'
* soft nofile 1048576
* hard nofile 1048576
EOF
...
EOF
    ...
    # Memory tuning
    cat >> /etc/sysctl.d/99-luk1s-mem.conf << 'EOF'
vm.swappiness=10
...
EOF
    ...
    sysctl -p /etc/sysctl.d/99-luk1s-mem.conf >/dev/null 2>&1
...
EOF
    ...
    cat > /etc/systemd/journald.conf.d/luk1s.conf << 'EOF'
[Journal]
SystemMaxUse=100M
Compress=yes
EOF
...
setup_monitoring() {
    ...
    # Status script
    cat > "${BIN_DIR}/luk1s-status" << 'SCRIPT'
#!/bin/bash
echo "=== luk1s Status $(date +%H:%M:%S) ==="
echo "Uptime: $(uptime -p)"
echo "Load: $(cat /proc/loadavg | cut -d' ' -f1-3)"
echo "RAM: $(free -m | awk 'NR==2{printf "%.0f%%", $3*100/$2}')"
echo "Disk: $(df -h / | awk 'NR==2{print $5}')"
...
SCRIPT
    ...
    chmod +x "${BIN_DIR}/luk1s-status"
    
    # Cron
    echo "*/${MONITOR_INTERVAL} * * * * root ${BIN_DIR}/luk1s-status >> ${MONITORING_LOG} 2>&1" > /etc/cron.d/luk1s
...
CMD
    ...
    # Log viewer
    cat > "${BIN_DIR}/luk1s-log" << 'LOG'
#!/bin/bash
LOG_FILE="/var/log/luk1s/status.log"
[[ -f "$LOG_FILE" ]] && tail -30 "$LOG_FILE" || echo "Нет логов"
LOG
    
    # Reset
    cat > "${BIN_DIR}/luk1s-reset" << 'RESET'
#!/bin/bash
rm -rf /etc/luk1s /var/log/luk1s /etc/cron.d/luk1s
rm -f /usr/local/bin/luk1s*
echo "Конфигурация удалена"
RESET
    
    chmod +x "${BIN_DIR}"/luk1s*
}
...
RESULT
    ...
    [[ "$ENABLE_MONITORING" == "yes" ]] && "${BIN_DIR}/luk1s-status"
